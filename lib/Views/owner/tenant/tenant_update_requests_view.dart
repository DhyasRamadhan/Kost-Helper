...
class TenantUpdateRequestsView extends StatefulWidget {
  ...
  @override
  State<TenantUpdateRequestsView> createState() => _TenantUpdateRequestsViewState();
}
...
class _TenantUpdateRequestsViewState extends State<TenantUpdateRequestsView> {
  ...
  Future<void> handleApprove(int id) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(result['success'] ? 'Request approved!' : result['message'] ?? 'Failed'),
  ...
  Future<void> handleReject(int id) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(result['success'] ? 'Request rejected' : result['message'] ?? 'Failed'),
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                Container(
                                  ...
                                  child: Text(
                                    req['status']?.toString().toUpperCase() ?? 'UNKNOWN',
                                    style: TextStyle(
                            ...
                            _buildInfoRow(Icons.arrow_back, 'Old Value', req['old_value'] ?? '-'),
                            const SizedBox(height: 8),
                            _buildInfoRow(Icons.arrow_forward, 'New Value', req['new_value'] ?? '-'),
                            if (isPending) ...[
                              const SizedBox(height: 16),
                              Row(
